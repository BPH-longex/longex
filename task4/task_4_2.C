...
// define namespace "semileptonic_bkg"
namespace semilep_bkg_pdf {
...
void task_4_2(unsigned int cate = 0, double bdt_min = 0.8)
{
    gROOT->SetBatch();

    RooWorkspace *wspace = new RooWorkspace("wspace","wspace");

    semilep_bkg_pdf::fill_dataset(wspace, cate);
    semilep_bkg_pdf::fit(wspace, cate);
    semilep_bkg_pdf::draw(wspace, cate);
