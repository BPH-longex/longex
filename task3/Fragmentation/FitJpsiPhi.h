void FitJpsiPhi(double ptMin, double ptMax, double etaMin, double etaMax, Int_t bin, Bool_t fixedPt)
{

    using namespace RooFit;	
    uint cate = 2;
    //if(fixedPt && etaMin>0.7) cate = 3;
    ...
    double minValueSigma;
    double maxValueSigma;
   ...
   // canvas1->Print("task3_2a.pdf");
    ...
    if(!fixedPt){
    canvas2->Print(Form("./output/FitJpsiPhi_%i_fixedEta.pdf", bin));
    }
    else if(fixedPt){
    canvas2->Print(Form("./output/FitJpsiPhi_%i_fixedPt.pdf", bin));
    }

    if(!fixedPt){
        hEffi_phi_fixedEta->SetBinContent(bin+1, eff);
        hEffi_phi_fixedEta->SetBinError(bin+1, eff_error);
        hYield_phi_fixedEta->SetBinError(bin+1, n_sig.getError());
        hYield_phi_fixedEta->SetBinContent(bin+1, n_sig.getVal());
    }
    else if(fixedPt){
        hEffi_phi_fixedPt->SetBinContent(bin+1, eff);
        hEffi_phi_fixedPt->SetBinError(bin+1, eff_error);
        hYield_phi_fixedPt->SetBinError(bin+1, n_sig.getError());
        hYield_phi_fixedPt->SetBinContent(bin+1, n_sig.getVal());
    }
